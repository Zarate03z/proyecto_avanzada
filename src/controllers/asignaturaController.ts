...
export const create = (asignatura: Asignatura, callback: Function) => {
    ...
    db.query(queryString, [asignatura.cod_a, asignatura.nom_a, asignatura.int_h, asignatura.creditos], (err) => {
        if (err) {
            callback(err);
        }
...
export const update = (asignatura: Asignatura, callback: Function) => {
    ...
    db.query(queryString, [asignatura.nom_a, asignatura.int_h, asignatura.creditos, asignatura.cod_a], (err) => {
        if (err) {callback(err);}
        callback(null, {
            statusCode: 200,
